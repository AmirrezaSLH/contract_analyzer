...
**Status: raised 2026-08-24, revised the same day for the React front end.**
A change request against `05_api_plan.md`, written from the UI in
`01_ui_spec.md`. Five items. Two block a surface, one is how the front end is
served at all, and two are cheap corrections that will cost more later than
now.

Nothing here contradicts a decision in the API plan — the shapes below extend
responses that already exist, and each one exists because a screen cannot be
drawn without it.
...
and stays there (`02_react_build.md` §6) — the API sees a number.
...
| 5 · `/api` prefix and static mount | **Everything** (13b) | one router prefix, one mount, one Dockerfile stage |
